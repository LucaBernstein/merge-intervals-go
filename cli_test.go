...
func TestLoadFromFile(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "[1,7][14,19][6,12]", LoadFromFile("test-interval-file.txt"))
}
...
func TestLoadFromArgs(t *testing.T) {
	...
	assert.Equal(t, "[1,2][3,4][7,8]", LoadInputIntervals())
}
...
func TestLoadIntervalsFallbackEnv(t *testing.T) {
	...
	os.Setenv(EnvIntervalsFile, "test-intervals-fallback-env.txt")

	err := os.WriteFile("test-intervals-fallback-env.txt", []byte("[ 100 , 700 ]"), os.ModePerm)
	assert.Nil(t, err)
	assert.Equal(t, "[100,700]", LoadInputIntervals())
}
...
func TestParseInputArgs(t *testing.T) {
	...
	output, err = ParseInput("[-1,2][-11,-3]")
	assert.Nil(t, err)
...
func BenchmarkParseInputArgs(b *testing.B) {
	...
	var err error
	for n := 0; n < b.N; n++ {
		intervals, err = ParseInput("[7,18][64000,128000][1,100000][2112,4224][0,1][25,117][1998,2401][2024,2025][15,20][117122,122117]")
	}
